...
# Fixed typo: 'widht' to 'width'
text_result = tk.Text(root, height=2, width=16, font=("Italic", 24))
...
btn_dot = tk.Button(root, text=".", command=lambda: add_to_calculation("."), width=5)
...
btn_modulus = tk.Button(root, text="%", command=lambda: add_to_calculation("%"), width=5)
btn_modulus.grid(row=1, column=3)
# Add buttons for operations, equals, and clear
btn_plus = tk.Button(root, text="+", command=lambda: add_to_calculation("+"), width=5)
btn_plus.grid(row=5, column=3)
btn_equals = tk.Button(root, text="=", command=evaluate_calculation, width=5)
